...
add_action('woocommerce_checkout_after_order_review', 'formAppend');

function some_custom_checkout_field_process_cartao() {

	if ($_POST['post_cartao']){
...
add_action( 'plugins_loaded', 'init_pjbank_getway_cartao' );
...
function your_custom_field_function_name($order){
	...
	$pj_cartao = get_post_meta( $order->id, '_pj_cartao');

	if($pj_cartao){
		
		$juros = get_post_meta( $order->id, '_juros');
		if($juros[0] > 0){
		...
		$parcelamento = get_post_meta( $order->id, '_parcelamento');
		$parcelas = get_post_meta( $order->id, '_parcelas' );
		if($parcelas[0] > 0){
		...
		$valor_total = get_post_meta( $order->id, '_valor_total' );
		if($valor_total[0] != null ){
